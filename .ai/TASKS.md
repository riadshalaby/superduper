...
| T-003 | Release notes from PR body sections | claude | codex | claude | ready_for_review | compose-release-notes.sh extracts PR Release Notes sections; CI uses composed notes with fallback chain; docs updated; build passes | `bash -n scripts/compose-release-notes.sh`; `scripts/compose-release-notes.sh v0.6.4-preview`; `mvn -q spotless:apply`; `mvn -q -DskipTests test-compile`; `mvn -T 1C -q test` | review |
