...
| T-003 | Release notes from PR body sections | claude | codex | claude | ready_for_implement | compose-release-notes.sh extracts PR Release Notes sections; CI uses composed notes with fallback chain; docs updated; build passes | n/a | implement |
| T-004 | Skip Maven Central for selected versions | claude | codex | claude | ready_for_implement | ai-release.sh --skip-central flips central.skipPublishing; visible in PR diff; docs updated; build passes | n/a | implement |
