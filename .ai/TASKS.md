...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T-001 | Unified CI Pipeline Consolidation: non-blocking Sonar, merge release.yml into ci.yml, strict job execution order, delete release.yml | claude | codex | claude | done | Sonar non-blocking on main; ci.yml has build+sonar+tag-version+release jobs; release.yml deleted; tag-version skips snapshots; release gated by is_release; ai-release.sh + CLAUDE.md updated | `mvn -q spotless:apply`; `mvn -q -DskipTests test-compile`; `mvn -T 1C -q test`; `bash -n scripts/ai-release.sh`; `git diff --check`; all 22 plan compliance items verified PASS | — |
