...
| T-003 | Test Coverage ≥ 80% | claude | codex | TBD | ready_for_review | Overall line coverage ≥80% (JaCoCo aggregate); each library module ≥70%; jacoco:check enforced in build; example modules excluded | Aggregate line coverage 89.26%; `repository-api` 93.51%; `starter-autoselect` 82.34%; `worker-blocking` 85.22%; validations passed: `mvn -q spotless:apply`, `mvn -q -DskipTests test-compile`, `mvn -T 1C -q test`, `mvn -q -DskipTests verify` | review |
