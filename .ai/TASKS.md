...
| T-004 | Skip Maven Central for selected versions | claude | codex | claude | done | ai-release.sh --skip-central flips central.skipPublishing; visible in PR diff; docs updated; build passes | `bash -n scripts/ai-release.sh`; `mvn -q spotless:apply`; `mvn -q -DskipTests test-compile`; `mvn -T 1C -q test`; live `prepare --skip-central` not run locally to avoid creating a real PR/push | — |
