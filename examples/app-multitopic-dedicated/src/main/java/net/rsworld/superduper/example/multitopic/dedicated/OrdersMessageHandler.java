...
@Component("ordersMessageHandler")
class OrdersMessageHandler implements MessageHandler {
    ...
    OrdersMessageHandler(SeedProgress progress) {
    ...
    public ProcessingResult handle(MessageRow row) {
        ...
        if (content.contains("always-fail")) {
            log.warn("[Orders] id={} key={} attempt={} -> FAILURE (always-fail)", row.id(), row.messageKey(), attempt);
            return ProcessingResult.FAILURE;
        }

        if (content.contains("retry-once") && attempt == 1) {
            log.info(
                    "[Orders] id={} key={} attempt={} -> FAILURE (retry-once first attempt)",
                    row.id(),
                    row.messageKey(),
                    attempt);
            return ProcessingResult.FAILURE;
        ...
        log.info(
                "[Orders] id={} key={} attempt={} -> SUCCESS content={}", row.id(), row.messageKey(), attempt, content);
