...
@Component("invoicesMessageHandler")
class InvoicesMessageHandler implements MessageHandler {
    private static final Logger log = LoggerFactory.getLogger(InvoicesMessageHandler.class);

    private final SeedProgress progress;
    private final Map<Long, Integer> attempts = new ConcurrentHashMap<>();

    InvoicesMessageHandler(SeedProgress progress) {
    ...
    public ProcessingResult handle(MessageRow row) {
        ...
        if (content.contains("always-fail")) {
            log.warn(
                    "[Invoices] id={} key={} attempt={} -> FAILURE (always-fail)", row.id(), row.messageKey(), attempt);
            return ProcessingResult.FAILURE;
        }

        if (content.contains("retry-once") && attempt == 1) {
            log.info(
                    "[Invoices] id={} key={} attempt={} -> FAILURE (retry-once first attempt)",
                    row.id(),
                    row.messageKey(),
                    attempt);
            return ProcessingResult.FAILURE;
        ...
        log.info(
                "[Invoices] id={} key={} attempt={} -> SUCCESS content={}",
                row.id(),
                row.messageKey(),
                attempt,
                content);
